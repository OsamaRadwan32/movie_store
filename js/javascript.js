...
const $errorMsg = $(
  '<span class="error-msg">Password length must be between 8 and 20.</span>'
);
...
$modalWindow.on("click", function (e) {
  if ($(e.target).is($modalWindow)) $modalWindow.removeClass("is-visible");
});
    ...
    for (var i = 1; i <= val; i++) {
      $(
        '<div class="form-group">' +
          '<label class="control-label ac">Author ' +
          i +
          " *</label>" +
...
$editBookBtn.on("click", function () {
  ...
  window.location.href =
    "http://localhost/TheMovieStore/admin/update-book.php?isbn=" + isbn;
});
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response == "updated") {
          ...
          setTimeout(function () {
            window.location.href =
              "http://localhost/TheMovieStore/admin/view-books.php";
          }, 2000);
        } else {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response == "updated") {
          ...
          setTimeout(function () {
            window.location.href =
              "http://localhost/TheMovieStore/admin/view-users.php";
          }, 2000);
        } else {
          $("#insertSuccessful").css("display", "none");
          $("#insertfail").css("display", "block");
      ...
      if (data.movies != undefined) {
        $.each(data.movies, function (index, movie) {
          ...
          var article =
            ...
            movie.name +
            "</a>" +
            '<p class="title"><strong>Release Year:</strong> ' +
            movie.release_year +
            "</p>" +
            ...
            movie.price +
            "</p>" +
            '<p class="title"><strong>Rating: </strong>' +
            movie.rating +
            "</p>" +
            "<a href='movie.php?id=" +
            movie.id +
            '\' class="reserve-btn">View more' +
            "</a></div></div></article>";

          $(article).appendTo("#article-wrapper");
        });
      } else {
        $('<div class="no-result">No Results</div>').appendTo(
          "#article-wrapper"
        );
      }
    },
...
$(function () {
  ...
  $genreList.each(function (index, element) {
    ...
    if (catalogURL == category) $(element).addClass("active");
  });
});
